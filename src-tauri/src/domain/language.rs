...
///
/// 跳过 `list_inner` 中的语言名称前缀行，只导出实际翻译条目。
pub fn language_document_to_table(document: &Value) -> TableDocument {
    ...
    let mut headers = vec!["序号".to_string(), "auto".to_string()];
    ...
    let rows = keys
        ...
        .enumerate()
        .map(|(index, key)| {
            let mut row = vec![(index + 1).to_string(), key.clone()];
...
pub fn parse_language_table(document: TableDocument) -> LanguageImportReport {
    ...
    let language_headers = language_headers_from_headers(&document.headers, &mut errors);
    ...
    if table.valid && errors.is_empty() {
        for (index, row) in document.rows.iter().enumerate() {
            ...
            for (language_index, code) in language_codes.iter().enumerate() {
                let value = cell(row, language_index + 2);
                values.insert(code.clone(), Value::String(value));
            }
            if language_codes.iter().any(|code| code == "zh") {
